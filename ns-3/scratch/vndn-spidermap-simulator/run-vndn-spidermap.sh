...
# 用途：从 ns-3 根目录重复运行 VNDN 网状道路仿真，并记录 OBU、RSU、Router 日志。
# Purpose: Repeatedly run the VNDN spider-map simulation from the ns-3 project root
#          with OBU, RSU, and Router logging enabled.
#
# 用法：./scratch/vndn-spidermap-simulator/run-vndn-spidermap.sh <运行次数>
# Usage: ./scratch/vndn-spidermap-simulator/run-vndn-spidermap.sh <run-count>
#
...
for ((run_index = 1; run_index <= run_count; ++run_index)); do
  echo "[$run_index/$run_count] Starting vndn-spidermap-simulator (300-second limit)"
  NS_LOG=ndn.VndnObu:ndn.VndnRsu:ndn.VndnRouter \
    timeout --foreground 300s ./waf --run "vndn-spidermap-simulator --log"
  exit_code=$?
